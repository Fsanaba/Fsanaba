<div align="center">

<img src="https://capsule-render.vercel.app/api?type=waving&color=gradient&customColorList=6,11,20&height=180&section=header&text=Francisco%20Sánchez%20Abad&fontSize=42&fontColor=fff&animation=twinkling&fontAlignY=32"/>

<h2>💻 Desarrollador Full Stack Junior | 🗄️ Especialista en Bases de Datos</h2>
<h3>🌱 Siempre aprendiendo nuevas tecnologías</h3>

<p>
  <img src="https://img.shields.io/badge/Ubicación-Osuna,_Sevilla-3B82F6?style=for-the-badge&logo=google-maps&logoColor=white"/>
  <img src="https://img.shields.io/badge/Disponibilidad-Inmediata-00D084?style=for-the-badge&logo=checkmarx&logoColor=white"/>
  <img src="https://img.shields.io/badge/Estado-Buscando_Empleo-FF6B6B?style=for-the-badge&logo=lighthouse&logoColor=white"/>
</p>

[![LinkedIn](https://img.shields.io/badge/LinkedIn-Conectar-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/tu-perfil)
[![Email](https://img.shields.io/badge/Email-Contacto-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:franciscosanaba913@gmail.com)
[![GitHub](https://img.shields.io/badge/GitHub-Sígueme-181717?style=for-the-badge&logo=github&logoColor=white)](https://github.com/TU_USERNAME)

</div>

---

## <img src="https://media.giphy.com/media/iY8CRBdQXODJSCERIr/giphy.gif" width="30px"> Sobre mí

<img align="right" alt="Coding" width="400" src="https://raw.githubusercontent.com/abhisheknaiidu/abhisheknaiidu/master/code.gif">

```javascript
const francisco = {
    rol: "Desarrollador Full Stack Junior",
    ubicacion: "Osuna, Sevilla 🇪🇸",
    educacion: "Técnico Superior en DAM",
    graduacion: 2025,
    pasiones: ["Bases de Datos", "Desarrollo Web", "UI/UX Design"],
    objetivos: ["Primera oportunidad laboral", "Aprendizaje continuo"],
    disponibilidad: "Inmediata ✅",
    
    skills: {
        lenguajes: ["Java", "PHP", "C#", "Python"],
        databases: ["MySQL", "Oracle", "SQL Server"],
        frontend: ["HTML", "CSS", "Angular"],
        tools: ["Git", "GitHub", "Figma", "Hibernate"],
        idiomas: ["🇪🇸 Español (Nativo)", "🇬🇧 Inglés (B2)", "🇫🇷 Francés (B1)"]
    },
    
    caracteristicas: {
        trabajoEnEquipo: true,
        resolucionProblemas: true,
        pensamientoCritico: true,
        adaptabilidad: true,
        comunicacionEfectiva: true
    }
};
```

<br clear="both">

---

## <img src="https://media2.giphy.com/media/QssGEmpkyEOhBCb7e1/giphy.gif?cid=ecf05e47a0n3gi1bfqntqmob8g9aid1oyj2wr3ds3mg700bl&rid=giphy.gif" width="28px"> Stack Tecnológico

<div align="center">

### 💻 Lenguajes de Programación

<table>
  <tr>
    <td align="center" width="96">
      <img src="https://techstack-generator.vercel.app/java-icon.svg" alt="Java" width="48" height="48" />
      <br>Java
    </td>
    <td align="center" width="96">
      <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/php/php-original.svg" width="48" height="48" alt="PHP" />
      <br>PHP
    </td>
    <td align="center" width="96">
      <img src="https://techstack-generator.vercel.app/csharp-icon.svg" alt="C#" width="48" height="48" />
      <br>C#
    </td>
    <td align="center" width="96">
      <img src="https://techstack-generator.vercel.app/python-icon.svg" alt="Python" width="48" height="48" />
      <br>Python
    </td>
  </tr>
</table>

### 🗄️ Bases de Datos

<table>
  <tr>
    <td align="center" width="96">
      <img src="https://techstack-generator.vercel.app/mysql-icon.svg" alt="MySQL" width="48" height="48" />
      <br>MySQL
    </td>
    <td align="center" width="96">
      <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/oracle/oracle-original.svg" width="48" height="48" alt="Oracle" />
      <br>Oracle
    </td>
    <td align="center" width="96">
      <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/microsoftsqlserver/microsoftsqlserver-plain.svg" width="48" height="48" alt="SQL Server" />
      <br>SQL Server
    </td>
  </tr>
</table>

### 🎨 Desarrollo Web & Frameworks

<table>
  <tr>
    <td align="center" width="96">
      <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/html5/html5-original.svg" width="48" height="48" alt="HTML5" />
      <br>HTML5
    </td>
    <td align="center" width="96">
      <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/css3/css3-original.svg" width="48" height="48" alt="CSS3" />
      <br>CSS3
    </td>
    <td align="center" width="96">
      <img src="https://techstack-generator.vercel.app/js-icon.svg" alt="JavaScript" width="48" height="48" />
      <br>JavaScript
    </td>
    <td align="center" width="96">
      <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/angularjs/angularjs-original.svg" width="48" height="48" alt="Angular" />
      <br>Angular
    </td>
  </tr>
</table>

### 🛠️ Herramientas & Control de Versiones

<table>
  <tr>
    <td align="center" width="96">
      <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/dot-net/dot-net-original.svg" width="48" height="48" alt=".NET" />
      <br>.NET
    </td>
    <td align="center" width="96">
      <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/hibernate/hibernate-original.svg" width="48" height="48" alt="Hibernate" />
      <br>Hibernate
    </td>
    <td align="center" width="96">
      <img src="https://techstack-generator.vercel.app/github-icon.svg" alt="GitHub" width="48" height="48" />
      <br>GitHub
    </td>
    <td align="center" width="96">
      <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/figma/figma-original.svg" width="48" height="48" alt="Figma" />
      <br>Figma
    </td>
  </tr>
</table>

</div>

---

## 💼 Experiencia Práctica

<div align="center">

<table>
  <tr>
    <td align="center">
      <img src="https://img.icons8.com/fluency/96/university.png" width="80"/>
    </td>
    <td align="left">
      <h3>💻 Desarrollador Web - Universidad de Osuna</h3>
      <p><strong>📅 Marzo 2025 - Junio 2025</strong> | 🎓 Prácticas Profesionales</p>
      <ul>
        <li>✅ Desarrollo de aplicación web para gestión de currículos de profesores</li>
        <li>✅ Programación en <strong>PHP</strong> y <strong>PhpMyAdmin</strong></li>
        <li>✅ Ciclo completo de desarrollo: análisis, diseño, implementación y pruebas</li>
        <li>✅ Trabajo con bases de datos relacionales</li>
      </ul>
    </td>
  </tr>
</table>

</div>

---

## 🌍 Idiomas

<div align="center">

| Idioma | Nivel | Certificación |
|:------:|:-----:|:-------------:|
| 🇪🇸 **Español** | Nativo | - |
| 🇬🇧 **Inglés** | B2 | Cambridge Certificate (2022) |
| 🇫🇷 **Francés** | B1 | DELF (2022) |

</div>

---

## 📊 Estadísticas de GitHub

<div align="center">

<img width="49%" src="https://github-readme-stats.vercel.app/api?username=TU_USERNAME&show_icons=true&theme=tokyonight&hide_border=true&bg_color=0D1117&title_color=3B82F6&icon_color=3B82F6&text_color=ffffff" />
<img width="49%" src="https://github-readme-streak-stats.herokuapp.com/?user=TU_USERNAME&theme=tokyonight&hide_border=true&background=0D1117&ring=3B82F6&fire=3B82F6&currStreakLabel=3B82F6" />

<br><br>

<img width="49%" src="https://github-readme-stats.vercel.app/api/top-langs/?username=TU_USERNAME&layout=compact&theme=tokyonight&hide_border=true&bg_color=0D1117&title_color=3B82F6&text_color=ffffff" />
<img width="49%" src="https://github-contributor-stats.vercel.app/api?username=TU_USERNAME&limit=5&theme=tokyonight&combine_all_yearly_contributions=true&hide_border=true&bg_color=0D1117&title_color=3B82F6&text_color=ffffff" />

</div>

---

## 🎓 Formación y Certificaciones

<div align="center">

<table>
  <tr>
    <td align="center" width="50%">
      <img src="https://img.icons8.com/color/96/graduation-cap.png" width="60"/>
      <h3>Técnico Superior en DAM</h3>
      <p><strong>IES Rodríguez Marín, Osuna</strong></p>
      <p>📅 2023-2025</p>
    </td>
    <td align="center" width="50%">
      <img src="https://img.icons8.com/color/96/certificate.png" width="60"/>
      <h3>Cambridge English B2</h3>
      <p><strong>Cambridge Assessment</strong></p>
      <p>📅 2022</p>
    </td>
  </tr>
  <tr>
    <td align="center">
      <img src="https://img.icons8.com/color/96/certificate.png" width="60"/>
      <h3>DELF Francés B1</h3>
      <p><strong>DELF, Osuna</strong></p>
      <p>📅 2022</p>
    </td>
    <td align="center">
      <img src="https://img.icons8.com/color/96/design.png" width="60"/>
      <h3>Curso de Figma</h3>
      <p><strong>OpenWebinars</strong></p>
      <p>📅 2024</p>
    </td>
  </tr>
</table>

</div>

---

## 📫 Contacto

<div align="center">

<a href="mailto:franciscosanaba913@gmail.com">
  <img src="https://img.shields.io/badge/Gmail-D14836?style=for-the-badge&logo=gmail&logoColor=white" alt="Gmail"/>
</a>
<a href="https://www.linkedin.com/in/tu-perfil">
  <img src="https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn"/>
</a>
<a href="https://github.com/TU_USERNAME">
  <img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white" alt="GitHub"/>
</a>
<a href="tel:+34609450090">
  <img src="https://img.shields.io/badge/Teléfono-25D366?style=for-the-badge&logo=whatsapp&logoColor=white" alt="Phone"/>
</a>

<br><br>

<table>
  <tr>
    <td align="center">
      <img src="https://img.icons8.com/fluency/96/email.png" width="50"/>
      <br><strong>Email</strong>
      <br>franciscosanaba913@gmail.com
    </td>
    <td align="center">
      <img src="https://img.icons8.com/fluency/96/phone.png" width="50"/>
      <br><strong>Teléfono</strong>
      <br>+34 609 450 090
    </td>
    <td align="center">
      <img src="https://img.icons8.com/fluency/96/marker.png" width="50"/>
      <br><strong>Ubicación</strong>
      <br>Osuna, Sevilla, España
    </td>
  </tr>
</table>

</div>

---

## 🤝 Soft Skills

<div align="center">

<table>
  <tr>
    <td align="center" width="33%">
      <img src="https://img.icons8.com/fluency/96/teamwork.png" width="60"/>
      <br><strong>Trabajo en Equipo</strong>
      <br>Colaboración efectiva en proyectos
    </td>
    <td align="center" width="33%">
      <img src="https://img.icons8.com/fluency/96/problem-solving.png" width="60"/>
      <br><strong>Resolución de Problemas</strong>
      <br>Pensamiento analítico y crítico
    </td>
    <td align="center" width="33%">
      <img src="https://img.icons8.com/fluency/96/learning.png" width="60"/>
      <br><strong>Aprendizaje Continuo</strong>
      <br>Adaptabilidad a nuevas tecnologías
    </td>
  </tr>
  <tr>
    <td align="center">
      <img src="https://img.icons8.com/fluency/96/communication.png" width="60"/>
      <br><strong>Comunicación Efectiva</strong>
      <br>Clara transmisión de ideas
    </td>
    <td align="center">
      <img src="https://img.icons8.com/fluency/96/settings.png" width="60"/>
      <br><strong>Adaptabilidad</strong>
      <br>Flexibilidad ante cambios
    </td>
    <td align="center">
      <img src="https://img.icons8.com/fluency/96/goal.png" width="60"/>
      <br><strong>Orientación a Resultados</strong>
      <br>Enfoque en objetivos claros
    </td>
  </tr>
</table>

</div>

---

<div align="center">

## 💡 Buscando mi Primera Oportunidad

<img src="https://img.shields.io/badge/Estado-Disponible_para_Trabajar-success?style=for-the-badge&logo=statuspage&logoColor=white" />
<img src="https://img.shields.io/badge/Modalidad-Presencial_|_Remoto_|_Híbrido-blue?style=for-the-badge&logo=googlehangoutsmeet&logoColor=white" />
<img src="https://img.shields.io/badge/Tipo-Junior_Developer_|_Prácticas-orange?style=for-the-badge&logo=codementor&logoColor=white" />

<br><br>

### 🎯 Estoy especialmente interesado en:
- 💾 Desarrollo y optimización de Bases de Datos
- 🌐 Desarrollo Web Full Stack
- 🎨 UI/UX y diseño de interfaces
- 📱 Aplicaciones Multiplataforma

<br>

<img src="https://komarev.com/ghpvc/?username=TU_USERNAME&label=Visitas%20al%20Perfil&color=3B82F6&style=for-the-badge" alt="Profile views" />

</div>

---

<div align="center">

<img src="https://capsule-render.vercel.app/api?type=waving&color=gradient&customColorList=6,11,20&height=100&section=footer"/>

<p><i>"El código es como el humor. Cuando tienes que explicarlo, es malo." – Cory House</i></p>

⭐️ **Si te gusta mi perfil, considera darle una estrella a mis repositorios** ⭐️

</div>
